...
################################################################################
# LOAD THE PACKAGES ******************************************************** ####
# Not every package is used, but these are great research packages
suppressMessages(pacman::p_load("PerformanceAnalytics","quantmod","purrr","rvest",
                                "LSMRealOptions","RQuantLib","fOptions","dplyr",
                                "anytime","pbapply","lubridate","TTR","tidyquant",
                                "httr","devtools","urca","derivmkts","tidyr",
...
DTE_YR_FRAC            <- function (OPTIONZ){
  ...
  ttt.tz       = if (Sys.timezone() == "US/Pacific"){"21:00"}else if (Sys.timezone() == "US/Central"){"23:00"}else if (Sys.timezone() == "US/Eastern"){"24:00"}
